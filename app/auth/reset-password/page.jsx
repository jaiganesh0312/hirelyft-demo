...
import { Button, Input, Card, CardHeader, CardBody, CardFooter, Link, Spinner } from "@heroui/react";
    ...
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-purple-50 to-pink-100 dark:from-gray-900 dark:to-black p-4">
      <Card className="w-full max-w-md shadow-xl bg-white/90 dark:bg-default-50/90 backdrop-blur-sm">
        <CardHeader className="flex flex-col items-center pt-6">
          ...
          <h1 className="text-2xl font-bold text-center text-gray-800 dark:text-gray-100">Reset Your Password</h1>
          <p className="text-sm text-gray-600 dark:text-gray-400 px-4 text-center mt-1">Enter your new password below.</p>
        </CardHeader>
        <form onSubmit={handleSubmit(onSubmit)}>
          <CardBody className="gap-4 pb-4">
            ...
            <Button
              ...
              fullWidth
              isLoading={loading}
              isDisabled={loading || !!successMessage || !token || !!error && !errors.password && !errors.confirmPassword} // Disable if loading, success, no token, or general error (but not validation errors)
          ...
          <CardFooter className="flex flex-col items-center pb-6">
             {successMessage ? (
                <p className="text-center text-sm text-gray-600 dark:text-gray-400">
                    Password reset successful! <Link size="sm" href="/auth/login">Sign In</Link>
                </p>
             ) : (
                <p className="text-center text-sm text-gray-600 dark:text-gray-400">
                    Need help? <Link size="sm" href="/auth/forgot-password">Request a new link</Link>
                </p>
             )}
          </CardFooter>
        </form>
...
export default function ResetPasswordPage() {
    return (
        <Suspense fallback={<div className="flex items-center justify-center min-h-screen"><Spinner label="Loading..."/></div>}>
