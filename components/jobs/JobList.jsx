...
function JobCard({ job, isAuthenticated }) {
    ...
    return (
        <Card shadow="sm" className="w-full hover:shadow-md transition-shadow duration-300">
            ...
            <CardBody className="pt-0">
                ...
                <div className="flex flex-wrap gap-2 text-xs mb-3">
                    {job.location_city && <Chip radius="sm" variant="bordered" color='primary' startContent={<Icon icon="mdi:map-marker-outline" />}>{job.location_city}{job.location_area ? `, ${job.location_area}` : ''}</Chip>}
                    {job.job_type && <Chip radius="sm" variant="bordered" color='secondary' startContent={<Icon icon="mdi:clock-outline" />} >{job.job_type}</Chip>}
                    {job.experience_level && <Chip radius="sm" variant="bordered" color='warning' startContent={<Icon icon="mdi:chart-line-variant" />}>{job.experience_level}</Chip>}
                    {(job.salary_min || job.salary_max) && <Chip radius="sm" variant="bordered" color='success' startContent={<Icon icon="mdi:cash" />} >{formatSalary(job.salary_min, job.salary_max)}</Chip>}
                </div>
            </CardBody>
            <CardFooter className="flex justify-between items-center">
                ...
                <Button 
                    as={Link} 
                    href={isAuthenticated ? `/jobs/${job.id}` : '/auth/login?redirect=/jobs'} 
                    color="primary" 
                    size="sm"
                    variant="ghost"
                >
